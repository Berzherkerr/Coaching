...
function Modal({ post, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 backdrop-blur-sm px-4 py-10 overflow-y-auto"
      ...
      <div className="relative bg-neutral-900 border border-neutral-800 rounded-sm w-full max-w-2xl p-7 sm:p-10 my-auto">
            ...
            {posts.map((post, i) => (
              <MotionReveal key={post.id} delay={i * 60}>
                <button type="button" onClick={() => setActive(post)}
                  className="group w-full text-left bg-neutral-900 border border-neutral-800 rounded-sm p-5 hover:-translate-y-1 hover:border-orange-500/40 hover:shadow-[0_18px_45px_rgba(0,0,0,0.45)] transition-all duration-300 flex flex-col h-full">
